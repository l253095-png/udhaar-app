...
// Turso Cloud Client Connection
const db = createClient({
  url: process.env.TURSO_DATABASE_URL,
  authToken: process.env.TURSO_AUTH_TOKEN,
});

// Async Initialization: Schema & Migrations Auto-Execution
  ...
  try {
    ...
    async function addColumnIfMissing(table, column, definition) {
      try {
        ...
        const existingColumns = result.rows.map((c) => c.name);
    ...
    // ---- 3. Fix ignored_sheet_names Schema ----
    const tableCheck = await db.execute(
      "SELECT 1 FROM sqlite_master WHERE type='table' AND name='ignored_sheet_names'"
    );
    if (tableCheck.rows.length > 0) {
      const colCheck = await db.execute('PRAGMA table_info(ignored_sheet_names)');
      const columns = colCheck.rows.map((c) => c.name);

      if (!columns.includes('normalized_name')) {
        console.log('[DB Migration] Rebuilding ignored_sheet_names schema for Turso...');
    ...
    console.log('✅ Turso Database schema & migrations initialized successfully!');
  } catch (err) {
